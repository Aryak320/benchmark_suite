...
CSV="scale_avg_semirings_tpch_second.csv"

echo "scale_factor,query,formula(s),counting(s),why(s)" > $CSV
#Query generation
 for QUERY in ${QUERIES[@]}
     do
     sed "0,/from/s// , sr_counting(provenance(), 'counting_map') from /" $DIRECTORY/${QUERY} > $DIRECTORY"/${QUERY%.*}_counting.sql"
     sed -i '1iSET search_path to public, provsql ;' $DIRECTORY"/${QUERY%.*}_counting.sql"


     sed "0,/from/s// , sr_formula(provenance(), 'formula_map') from /" $DIRECTORY/${QUERY} > $DIRECTORY"/${QUERY%.*}_formula.sql"
     sed -i '1iSET search_path to public, provsql ;' $DIRECTORY"/${QUERY%.*}_formula.sql"


     sed "0,/from/s// , sr_why(provenance(), 'why_map') from /" $DIRECTORY/${QUERY} > $DIRECTORY"/${QUERY%.*}_why.sql"
     sed -i '1iSET search_path to public, provsql ;' $DIRECTORY"/${QUERY%.*}_why.sql"
    ...
    for QUERY in ${QUERIES[@]}
     do
          ...
          do
             ...
             START=$(date +%s.%N)
             PGPASSWORD=$PASSWORD psql -U $USER -h $HOST -d $DATABASE -f $DIRECTORY"/${QUERY%.*}_formula.sql" -o $OUTPUT_DIR"${QUERY%.*}_formula_output_${sf}.txt"
             END=$(date +%s.%N)
          ...
          do
            echo "iteration counting $i"
             START=$(date +%s.%N)
             PGPASSWORD=$PASSWORD psql -U $USER -h $HOST -d $DATABASE -f $DIRECTORY"/${QUERY%.*}_counting.sql" -o $OUTPUT_DIR"${QUERY%.*}_counting_output_${sf}.txt"
             END=$(date +%s.%N)
          ...
          do
             ...
             START=$(date +%s.%N)
             PGPASSWORD=$PASSWORD psql -U $USER -h $HOST -d $DATABASE -f $DIRECTORY"/${QUERY%.*}_why.sql" -o $OUTPUT_DIR"${QUERY%.*}_why_output_${sf}.txt"
